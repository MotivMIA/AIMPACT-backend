...
docker build --platform linux/amd64 -t aim-backend:amd64 . || {
...
docker tag aim-backend:amd64 "$DOCKERHUB_USERNAME/aim-backend:amd64" || {
  echo "Error: Docker tag failed."
  exit 1
}
docker push "$DOCKERHUB_USERNAME/aim-backend:amd64" || {
...
curl -X POST \
  ...
  -d "{\"serviceId\": \"$RENDER_SERVICE_ID\", \"image\": \"$DOCKERHUB_USERNAME/aim-backend:amd64\"}" \
